...
class FetchPage extends React.Component {
  ...
  render() {
    return (
      <div>
        <h1>This page was used to communicate to json data from localhost:3000/json (GET or POST) with Redux</h1>
        <div>Recieved data: {this.props.user.name}</div>
        <button onClick={() => {this.props.getName()}}>Click here to get data</button>
        <form onSubmit={this.onSubmit.bind(this) } >
          <input size="100" placeholder="Type somthing for POST data to server" onChange={this.onChange.bind(this)} />
